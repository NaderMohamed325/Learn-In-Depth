...
/**
 * @brief EXTI GPIO Mapping structure
 * This structure is used to map a specific EXTI line to a GPIO port and pin.
 */
typedef struct
{
    uint16_t EXTI_InputLine_Number;  ///< Specifies the EXTI line number (EXTI0 to EXTI15)
    GPIO_TypeDef *GPIOx;             ///< Pointer to the GPIO port (GPIOA, GPIOB, GPIOC, GPIOD)
    uint16_t GPIO_Pin;               ///< Specifies the GPIO pin number (e.g., GPIO_PIN_0)
    uint8_t IVT_IRQ_Number;          ///< Specifies the IRQ number in the interrupt vector table
} EXTI_GPIO_Maping_t;

/**
 * @brief EXTI Pin Configuration structure
 * This structure is used to configure a specific EXTI pin.
 */
typedef struct
{
    EXTI_GPIO_Maping_t EXI_Pin;      ///< Specifies the external interrupt mapping
    uint8_t Trigger_Case;            ///< Specifies the trigger case (Rising, Falling, Rising and Falling)
    uint8_t IRQ_EN;                  ///< Enable or Disable EXTI IRQ (controls IRQ_MASK in EXTI and NVIC interrupt)
    void (*P_IRQ_CallBack)(void);    ///< Pointer to the IRQ callback function
} EXTI_PinConfig_t;
...
// EXTI GPIO mapping definitions for GPIOA
...
// EXTI GPIO mapping definitions for GPIOB
...
// EXTI GPIO mapping definitions for GPIOC
...
// EXTI GPIO mapping definitions for GPIOD
...
// EXTI IRQ enable/disable macros
