...
{
    uint8_t index = (SPIx == SPI1) ? SPI1_INDEX : SPI2_INDEX;

    if (Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_2LINES ||
        Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_1LINE_transmit_only)
    {
    ...
    {
        // TODO: handle other communication modes
    }
}
...
void MCAL_SPI_ReceiveData(SPI_TypeDef *SPIx, uint16_t *pTxBuffer, enum PollingMechism PollingEn)
{
    ...
    if (Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_2LINES ||
        Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_2LINES_RXONLY)
    {
        if (PollingEn == Pollingenable)
            while (!((SPIx)->SPI_SR & SPI_SR_RXNE));
